...
def fetch_lemmas_for_level(cursor, level, count):
    ...
    if max_rank is None:
        cursor.execute(
            """
            SELECT rl.rank, rl.lemma, rl.pos, rl.translation, rl.definition
            FROM russian_lemmas rl
            WHERE rl.rank >= %s
              AND NOT EXISTS (
                  SELECT 1
                  FROM russian_vocab_test_bank q
                  WHERE q.lemma_rank = rl.rank
              )
              AND rl.translation IS NOT NULL
              AND rl.definition IS NOT NULL
            ORDER BY RANDOM()
            LIMIT %s
            """,
            [min_rank, count]
...
def build_prompt(lemma_rows):
    ...
    return f"""
You are creating high-quality Russian vocabulary diagnostic questions for intermediate and advanced learners.

For each Russian lemma, create one multiple-choice fill-in-the-blank question entirely in Russian.
...
- The question must be a natural Russian sentence with exactly one blank.
...
- The correct_answer must be the target Russian lemma, or the most natural inflected form if the sentence requires inflection.
...
The ambiguity_check field must be a short Russian or English explanation of why only the correct answer fits naturally.
...
def validate_generated_questions(results, expected_ranks):
    ...
    for item in results:
        ...
        # Simple local sanity checks for Russian fill-in-the-blank quality.
...
def build_review_prompt(generated_questions):
    return f"""
You are reviewing Russian fill-in-the-blank vocabulary diagnostic questions.
...
def insert_questions(cursor, generated_questions):
    ...
    for item in generated_questions:
        cursor.execute(
            """
            INSERT INTO russian_vocab_test_bank (
                lemma_rank,
                question,
                correct_answer,
                distractor_1,
